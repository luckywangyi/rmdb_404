...
std::unique_ptr<RmRecord> RmFileHandle::get_record(const Rid& rid, Context* context) const {
    ...
    record->data = page_handle.get_slot(rid.slot_no);  // 获取记录数据
...
Rid RmFileHandle::insert_record(char* buf, Context* context) {
    ...
    if (slot_no == -1) {
        throw std::runtime_error("No free slot available in the page.");
    }
    ...
    // 如果页面已满，更新file_hdr_.first_free_page_no
...
void RmFileHandle::delete_record(const Rid& rid, Context* context) {
    ...
    // 2. 检查目标位置是否存在记录
    if (!Bitmap::is_set(page_handle.bitmap, rid.slot_no)) {
        throw std::runtime_error("No record exists at the specified slot.");
    }

    // 3. 更新page_handle.page_hdr中的数据结构
    Bitmap::reset(page_handle.bitmap, rid.slot_no);  // 将目标位置的位图置为0
    page_handle.page_hdr->num_records--;            // 减少记录计数

    // 4. 如果页面从满状态变为未满状态，更新文件头和页头
